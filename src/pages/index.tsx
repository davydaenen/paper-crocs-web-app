...
const Home: NextPage = () => {
  ...
  useEffect(() => {
    ...
    if (data?.crocsWithPaper && data?.crocsWithPaper?.length) {
      const sortedCrocs = data.crocsWithPaper.sort((a, b) =>
        a?.priceInfo?.price.localeCompare(b?.priceInfo?.price)
      );

      setPaperCrocs(sortedCrocs);
      console.log("cheapest croc", sortedCrocs[0]);
    }
  }, [data]);
            ...
            <div className="flex flex-col justify-center items-center md:flex-row w-full md:w-4/5 xl:w-3/5 2xl:w-1/2 m-auto">
              <FloorPriceCard
                ...
                floorPrice={ethers.utils.formatEther(
                  paperCrocs[0]?.priceInfo?.price
                )}
                currency={paperCrocs[0]?.paymentToken?.symbol ?? "ETH"}
                footer={paperCrocs[0]?.marketplace ?? ""}
                footerUrl={paperCrocs[0]?.url ?? ""}
              />
            </div>
          </>
        ) : (
